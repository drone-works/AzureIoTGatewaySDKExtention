...
typedef struct SIMULATEDDEVICE_DATA_TAG
{
    ...
    THREAD_HANDLE       udp2cThread;
    const char *        fakeMacAddress;
    unsigned int        messagePeriod;
    unsigned int        udp2cRunning : 1;
...
static void udp2c_Receive(MODULE_HANDLE moduleHandle, MESSAGE_HANDLE messageHandle)
{
...
static void udp2c_Destroy(MODULE_HANDLE moduleHandle)
{
    ...
    else
    {
        ...
        /* Tell thread to stop */
        module_data->udp2cRunning = 0;
        /* join the thread */
        ThreadAPI_Join(module_data->udp2cThread, &result);
        /* free module data */
...
static int udp2c_worker(void * user_data)
{
    ...
    if (user_data != NULL)
    {
        ...
        while (module_data->udp2cRunning)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (receive_udp(buff_r) ==  0 && buff_r != NULL) 
                    { 
                        len = strlen(buff_r);

                        avgTemperature = atof(buff_r);
...
static void udp2c_Start(MODULE_HANDLE moduleHandle)
{
    if (moduleHandle == NULL)
    {
    ...
    else
    {
        ...
        if (ThreadAPI_Create(
            &(module_data->udp2cThread),
            udp2c_worker,
            (void*)module_data) != THREADAPI_OK)
        {
            LogError("ThreadAPI_Create failed");
            module_data->udp2cThread = NULL;
        }
        else
        {
...
static MODULE_HANDLE udp2c_Create(BROKER_HANDLE broker, const void* configuration)
{
...
static void * udp2c_ParseConfigurationFromJson(const char* configuration)
{
...
void udp2c_FreeConfiguration(void * configuration)
{
	if (configuration != NULL)
	{
        SIMULATEDDEVICE_CONFIG * config = (SIMULATEDDEVICE_CONFIG *)configuration;
...
static const MODULE_API_1 udp2c_APIS_all =
{
    {MODULE_API_VERSION_1},

	udp2c_ParseConfigurationFromJson,
	udp2c_FreeConfiguration,
    udp2c_Create,
    udp2c_Destroy,
    udp2c_Receive,
    udp2c_Start
...
//    return (const MODULE_API *)&SimulatedDevice_APIS_all;
    return (const MODULE_API *)&udp2c_APIS_all;
}
